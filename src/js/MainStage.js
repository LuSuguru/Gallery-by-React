...
class MainStage extends Component {
  ...
  rearrange(centerIndex) {
    ...
    let topImgSpliceIndex = 0 // 取出的元素在数组的位置
  ...
  componentDidMount() {
    const stateDom = ReactDOM.findDOMNode(this.refs.state) // 首先拿到舞台的大小
    const stateW = stateDom.scrollWidth
    const stateH = stateDom.scrollHeight

    const halfStateW = Math.ceil(stateW / 2)
    const halfStateH = Math.ceil(stateH / 2)

    const imgFigureDom = ReactDOM.findDOMNode(this.refs.imgFigure0) // 拿到一个imageFigure的大小
    const imgFigureW = imgFigureDom.scrollWidth
    const imgFigureH = imgFigureDom.scrollHeight
